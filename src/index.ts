...
export async function generate(source: Source, octokit: Octokit): Promise<Listing> {
  assertSource(source);
  ...
  for (const githubRepo of githubRepos) {
    ...
    for (const release of releases) {
      const packageJson = release.assets.find(asset => asset.name === "package.json");
      ...
      if (!zip) throw new Error(`Failed to find zip file ${zipName} in release ${githubRepo} ${release.name}`);
...
async function fetchPackageJson(url: string) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch package.json from ${url}`);
  return assertPackage(await res.json());
}

async function fetchZipSHA256(url: string) {
  const res = await fetch(url, { redirect: "follow" });
  if (!res.ok) throw new Error(`Failed to fetch zip file from ${url}`);
